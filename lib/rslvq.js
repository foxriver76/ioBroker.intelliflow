...
class RSLVQ {
    ...
    async optimize(X, y) {
        ...
        if (highestCostCorr < highestCostIncorr) {
            await this.updatePrototype(y=y, label=labelIncorr, proto=protoIncorr);
            await this.updatePrototype(y=y, label=labelCorr, proto=protoCorr);
        } // endIf
    } // endOptimize

    async updatePrototype(y, label, proto) {
