...
class RSLVQ {
    ...
    constructor(options) {
        ...
        if (this.gradientOptimizer === `rslvq-adadelta`) {
            ...
            this.logger.error(`Unknown gradient optimizer: ${this.gradientOptimizer} using Adadelta`);
            this.updatePrototype = this.updatePrototypeAdadelta;
        } // endElse
    ...
    async validatePrototypes(X, y) {
        ...
        if (this.initialPrototypes) {
            ...
            this.pastGradientsM = this.initialPrototypes.pastGradientsM;
            this.pastSquaredGradientsV = this.initialPrototypes.pastSquaredGradientsV;
            ...
            for (let i = 0; i < this.prototypesPerClass; i++) {
                ...
                if (!this.pastGradientsM[y]) this.pastGradientsM[y] = {};
                this.pastGradientsM[y][i] = nj.zeros(X.tolist().length);
                if (!this.pastSquaredGradientsV[y]) this.pastSquaredGradientsV[y] = {};
                this.pastSquaredGradientsV[y][i] = nj.zeros(X.tolist().length);
            } // endFor
        } // endElse
        ...
        */
        for (const label in this.w) {
            for (const proto in this.w[label]) {
                ...
                */
                // learn every prototype
                await this.updatePrototype(y, parseInt(label), X, proto);
            } // endFor
        } // endFor
    ...
    async updateProtoypeAdamax(y, label, x, protoIdx) {
        ...
        const broadcastedBeta2 = [];
        const broadcastedBeta1 = [];
        const broadcastedOnes = [];
        const broadcastedLearningRate = [];
        const broadcastedEpsilon = [];
        const broadcastedBeta1Rest = [];
        for (let i = 0; i < nj.array(this.w[y][protoIdx]).size; i++) {
            broadcastedBeta2[i] = this.beta2;
            broadcastedBeta1[i] = this.beta1;
            broadcastedOnes[i] = 1;
            broadcastedLearningRate[i] = this.learningRate;
            broadcastedEpsilon[i] = this.epsilon;
            broadcastedBeta1Rest[i] = 1 - this.beta1;
        } // endFor

        this.pastGradientsM[label][protoIdx] = nj.add(nj.multiply(broadcastedBeta1, this.pastGradientsM[label][protoIdx]), nj.multiply(broadcastedBeta1Rest, gradient));
        this.pastSquaredGradientsV[label][protoIdx] = this.maximum(nj.multiply(broadcastedBeta2, this.pastSquaredGradientsV[label][protoIdx]), nj.abs(gradient));

        // Bias correction
        const mHat = nj.divide(this.pastGradientsM[label][protoIdx], nj.subtract(broadcastedOnes, this.beta1));

        // perform update
        this.logger.warn(`old proto: ${this.w[label][protoIdx]}`);
        this.w[label][protoIdx] = nj.multiply(nj.divide(broadcastedLearningRate, nj.add(this.pastSquaredGradientsV[label][protoIdx], broadcastedEpsilon)), mHat);
    ...
    maximum(X1, X2) {
        ...
        let abs2 = 0;
        // iterate over X1
        for (let i = 0; i < nj.array(X1).size; i++) {
            abs1 += nj.array(X1).get(i);
        }

        // iterate over X2
        for (let i = 0; i < nj.array(X2).size; i++) {
            abs2 += nj.array(X2).get(i);
        }

        return abs1 < abs2 ? X2 : X1;
    } // endMaximum
